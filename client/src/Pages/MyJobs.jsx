...
const MyJobs = () => {
    const email = "dulajupananda@gmail.com";
    ...
    useEffect(() => {
        setIsLoading(true)
        fetch(`http://localhost:3000/myJobs/dulajupananda@gmail.com`)
        .then(res => res.json())
        .then(data => {
            setJobs(data);
            setIsLoading(false);
  ...
  return (
    <div className='max-w-screen-2xl container mx-auto xl:px-24 px-4'>
        <div className='my-jobs-conainer'>
            <h1 className='text-center'>All My Jobs</h1>
            <div className='search-box p-2 text-center mb-2'>
                <input
                onChange={(e) => setSearchText(e.target.value)}
                 type='text' name='search' id="search" className='py-2 mt-4 pl-3 border focus:outline-none lg:w-6/12 mb-4 w-full'/>
        ...
        <div className="relative flex flex-col min-w-0 break-words bg-white w-full mb-6 shadow-lg rounded ">
            ...
            <div className="flex flex-wrap items-center">
                ...
                <Link to="/post-job"><button className="bg-indigo-500 text-white active:bg-indigo-600 text-xs font-bold uppercase px-3 py-1 rounded outline-none 
                focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150" type="button">Post A New Job</button></Link>
                </div>
            </div>
            </div>
